...
function t4() {
    let a4_res = a4.filter((elem) => { if (typeof(elem) == "number") { return true } })
    return a4_res
}
...
function t8() {
    let a8_res = []
    let a8_res_prepare = a8.filter((elem, index) => {
        if (elem % 2 == 0) {
            a8_res.push(index)
            return true
        }
...
function t10() {
    let a10_res = []
    a10.forEach(function(elem, index) {
        if (elem < 0) {
            a10_res.push(0)
        } else {
            a10_res.push(elem)
...
function t11() {
    ...
    let a11_res = a11.filter((elem, index) => index % 2 == 0)
    return a11_res
}
...
function t15() {
    let a15_res = a15.filter(elem => elem.pnum.length)
    return a15_res
}
